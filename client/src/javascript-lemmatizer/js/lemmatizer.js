...
import * as noun_idx from '../dict/index.noun.json'
import * as verb_idx from '../dict/index.verb.json'
import * as adj_idx from '../dict/index.adj.json'
import * as adv_idx from '../dict/index.adv.json'
import * as pronoun_idx from '../dict/index.pronoun.json'
import * as conj_idx from '../dict/index.conj.json'
import * as prep_idx from '../dict/index.prep.json'
import * as article_idx from '../dict/index.article.json'

import * as noun_exc from '../dict/noun.exc.json'
import * as verb_exc from '../dict/verb.exc.json'
import * as adj_exc from '../dict/adj.exc.json'
import * as adv_exc from '../dict/adv.exc.json'
...
Lemmatizer.prototype = {
  ...
  lemmas: function(form, pos) {
    ...
    var parts = ['verb', 'noun', 'adj', 'adv', 'pronoun', 'conj', 'prep', 'article'];
    if ( pos && !parts.includes(pos)) {
      console.log("warning: pos must be 'verb' or 'noun' or 'adj' or 'adv' or 'pronoun' or 'conj' or 'prep' or 'article'.");
      return;
    }
    ...
    return this.uniq_lemmas(this.lems).sort(function(val) { return val[1]; } ).reverse();
  },
  ...
  only_lemmas: function(form, pos) {
    ...
    var result = this.lemmas(form, pos).map(function(val) { return val[0]; } );
    //return _.uniq(result);
    return result.filter((value, index, self) => { return self.indexOf(value) === index; })
  },
  ...
  setup_dic_data: function() {
    var self = this;
    noun_idx.default.forEach(function (w) {
      self.wordlists['noun'][w] = w;
    });
    verb_idx.default.forEach(function (w) {
      self.wordlists['verb'][w] = w;
    });
    adj_idx.default.forEach(function (w) {
      self.wordlists['adj'][w] = w;
    });
    adv_idx.default.forEach(function (w) {
      self.wordlists['adv'][w] = w;
    });
    pronoun_idx.default.forEach(function (w) {
      self.wordlists['pronoun'][w] = w;
    });
    conj_idx.default.forEach(function (w) {
      self.wordlists['conj'][w] = w;
    });
    prep_idx.default.forEach(function (w) {
      self.wordlists['prep'][w] = w;
    });
    article_idx.default.forEach(function (w) {
      self.wordlists['article'][w] = w;
    });

    noun_exc.default.forEach(function (item) {
      var w = item[0];
      var s = item[1];
      self.exceptions['noun'][w] = s;
    });
    verb_exc.default.forEach(function (item) {
      var w = item[0];
      var s = item[1];
      self.exceptions['verb'][w] = s;
    });
    adj_exc.default.forEach(function (item) {
      var w = item[0];
      var s = item[1];
      self.exceptions['adj'][w] = s;
    });
    adv_exc.default.forEach(function (item) {
      var w = item[0];
      var s = item[1];
      self.exceptions['adv'][w] = s;
    });
  },
